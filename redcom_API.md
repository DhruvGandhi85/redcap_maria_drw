...
```python
send_email(message: 'str' = '', recipient_emails: 'list' = ['']) → str
```
...
```python
send_error_email(message: 'str' = '', recipient_emails: 'list' = ['']) → str
```
...
create_data_res_workflow_entry(
    ...
    ping: 'bool',
    comment: 'str',
    ts: 'datetime.datetime' = datetime.datetime(2024, 12, 17, 19, 51, 42, 458201, tzinfo=datetime.timezone.utc)
) → None
```
...
## <kbd>function</kbd> `data_workflow_submission`

```python
data_workflow_submission(
    project_id: 'int',
    event_id: 'int',
    hnrcid: 'int',
    form_name: 'str',
...
```python
submit_stored_drw_entries() → None
```
